...
class ModelTrainer:
    ...
    def prepare_dataset(
        ...
        if len(findings) != len(labels):
            raise ValueError(
                f"Findings ({len(findings)}) and labels ({len(labels)}) must have same length"
            )
    ...
    def train_baseline(
        ...
        _ = np.digitize(y_test, bins=[25, 50, 75])  # y_test_class not used
        ...
        metrics = self._calculate_metrics(
            model, X_train, y_train_class, X_test, y_test, y_pred
        )
    ...
    def train_random_forest(
        ...
        logger.info(
            f"Random Forest trained: RMSE={metrics.rmse:.2f}, R²={metrics.r2:.3f}"
        )
