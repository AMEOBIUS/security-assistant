...
class MLScorer:
    ...
    def _get_confidence_interval(
        self, X: np.ndarray, prediction: float, confidence: float = 0.95
    ) -> Tuple[float, float]:
        """
    ...
    def explain_score(self, ml_score: MLScore, top_n: int = 5) -> str:
        ...
        # Get top features
        top_features = sorted(
            ml_score.feature_importance.items(), key=lambda x: abs(x[1]), reverse=True
        )[:top_n]
    ...
    def compare_with_baseline(
        self, finding: UnifiedFinding, baseline_score: float
    ) -> Dict[str, Any]:
        """
