...
class GitLabAPI:
    ...
    def __init__(
        ...
        if not self.private_token:
            raise GitLabAuthError(
                "GitLab token not provided. Set GITLAB_TOKEN or GITLAB_PERSONAL_TOKEN environment variable."
            )
    ...
    def _create_session(self) -> requests.Session:
        ...
        session.headers.update(
            {"PRIVATE-TOKEN": self.private_token, "Content-Type": "application/json"}
        )
    ...
    def create_issue(self, project_id: str, issue_data: IssueData) -> Dict[str, Any]:
        """
    ...
    def list_issues(
        self, project_id: str, labels: Optional[List[str]] = None, state: str = "opened"
    ) -> List[Dict[str, Any]]:
        """
