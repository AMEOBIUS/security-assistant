    ...
    def generate(self, result: Any, **kwargs) -> str:
        ...
        for finding in result.deduplicated_findings:
            ...
            rule_id = (
                finding.cwe_ids[0]
                if finding.cwe_ids
                else f"SEC-{finding.scanner.value.upper()}-001"
            )
            ...
            if rule_id not in rule_indices:
                rule = {
                    ...
                    "help": {
                        "text": finding.fix_guidance
                        or "No remediation advice available.",
                        "markdown": f"{finding.description}\n\n## Remediation\n{finding.fix_guidance or 'No remediation advice available.'}",
                    },
                    "properties": {
                        "tags": ["security", finding.scanner.value],
                    ...
                    "invocations": [
                        {
                            ...
                            "endTimeUtc": result.scan_time.isoformat()
                            if result.scan_time
