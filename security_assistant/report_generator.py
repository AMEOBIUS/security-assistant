...
try:
    # from .remediation.advisor import RemediationAdvisor  # Not used yet
    ...
    def _generate_bulk_json(self, bulk_result: BulkScanResult, output_path: Path):
        ...
        for target, result in bulk_result.results.items():
            data["targets"][target] = {
                ...
                "findings": [
                    self._finding_to_dict(f) for f in result.deduplicated_findings
                ],
            }
...
<body>
    ...
    def _finding_to_dict(self, finding: UnifiedFinding) -> Dict[str, Any]:
        ...
        reporter = ReporterFactory.create(
            "json", include_code_snippets=self.include_code_snippets
        )
