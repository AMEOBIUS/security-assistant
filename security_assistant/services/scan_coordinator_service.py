...
class ScanCoordinatorService:
    ...
    def scan_directory(
        ...
        return self._run_scanners_parallel(directory, recursive, is_file=False)
    ...
    def scan_file(self, file_path: str) -> Dict[ScannerType, Any]:
        ...
        return self._run_scanners_parallel(file_path, recursive=False, is_file=True)
            ...
            for scanner_type in self._enabled_scanners:
                ...
                if is_file:
                    future = executor.submit(
                        self._run_scanner_file_safe, scanner_type, scanner, target
                    )
                else:
                    future = executor.submit(
                        self._run_scanner_safe, scanner_type, scanner, target, recursive
                    )
