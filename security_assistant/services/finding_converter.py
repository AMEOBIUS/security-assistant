...
class FindingConverter:
    ...
    def convert_all(
        self, scanner_results: Dict[ScannerType, Any], unified_finding_class
    ) -> List:
        """
        ...
        for scanner_type, result in scanner_results.items():
            ...
            try:
                ...
                logger.debug(
                    f"Converted {len(findings)} findings from {scanner_type.value}"
                )
    ...
    def convert_bandit(self, result, unified_finding_class) -> List:
        ...
        for finding in result.findings:
            ...
            line_end = (
                finding.line_number + line_range - 1
                if line_range > 1
            ...
            if hasattr(finding, "cwe_ids"):
                cwe_ids = (
                    finding.cwe_ids
                    if isinstance(finding.cwe_ids, list)
                    else [finding.cwe_ids]
                )
            ...
            unified_finding = unified_finding_class(
                ...
                severity=self.BANDIT_SEVERITY_MAP.get(
    ...
    def convert_semgrep(self, result, unified_finding_class) -> List:
        ...
        for finding in result.findings:
            ...
            code_snippet = getattr(finding, "lines", None) or getattr(
                finding, "code", ""
            )
            ...
            if (
                not references
                and hasattr(finding, "metadata")
                and isinstance(finding.metadata, dict)
            ):
            ...
            unified_finding = unified_finding_class(
                ...
                severity=self.SEMGREP_SEVERITY_MAP.get(
                    finding.severity, FindingSeverity.MEDIUM
                ),
    ...
    def convert_trivy(self, result, unified_finding_class) -> List:
        ...
        for finding in result.findings:
            ...
            vuln_id = (
                ...
                or finding.secret_id
                or finding.misconfig_id
                or "unknown"
            )
            ...
            unified_finding = unified_finding_class(
                ...
                severity=self.TRIVY_SEVERITY_MAP.get(
                    finding.severity, FindingSeverity.MEDIUM
                ),
                category=category,
