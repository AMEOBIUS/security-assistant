    ...
    def _build_description(self, finding) -> str:
        ...
        if finding.is_active_exploit:
            description_parts.append(
                "**⚠️ ACTIVE EXPLOIT (KEV):** This vulnerability is actively exploited in the wild!"
            )

        if finding.is_false_positive:
            description_parts.append(
                f"**🤔 Possible False Positive:** Confidence {finding.fp_confidence:.0%}"
            )
            if finding.fp_reasons:
                description_parts.append(
                    f"  - Reasons: {', '.join(finding.fp_reasons)}"
                )

        if finding.is_reachable is False:
            description_parts.append(
                f"**📦 Unreachable Dependency:** Confidence {finding.reachability_confidence:.0%}"
            )
        ...
        if finding.ml_score is not None:
            ...
            if finding.epss_score is not None:
                description_parts.append(
                    f"**EPSS Score:** {finding.epss_score:.2%} (exploit probability)"
                )
