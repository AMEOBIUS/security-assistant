...
class BaseScanner(ABC, Generic[F, R]):
    ...
    def __init__(self, config: Optional[ScannerConfig] = None, **kwargs):
        ...
        logger.info(f"{self.name} scanner initialized (min_severity={self.config.min_severity})")
    ...
    def scan_directory(self, directory: str, recursive: bool = True) -> R:
        ...
        logger.info(f"[{self.name}] Scanning directory: {directory} (recursive={recursive})")
