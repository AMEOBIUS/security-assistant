...
class ComparisonResult:
    ...
    def has_new_critical(self) -> bool:
        ...
        return any(
            f.finding.severity == FindingSeverity.CRITICAL for f in self.new_findings
        )
    ...
    def has_new_high(self) -> bool:
        ...
        return any(
            f.finding.severity == FindingSeverity.HIGH for f in self.new_findings
        )
    ...
    def high_fixed(self) -> int:
        ...
        return sum(
            1 for f in self.fixed_findings if f.finding.severity == FindingSeverity.HIGH
        )
...
class ReportComparator:
    ...
    def compare(
        ...
        for key, latest_finding in latest_map.items():
            if key not in baseline_map:
                ...
                if (
                    self.track_severity_changes
                    and baseline_finding.severity != latest_finding.severity
                ):
                    # Severity changed
                    diff = FindingDiff(
                        finding=latest_finding,
        ...
        result.baseline_severity_counts = self._count_by_severity(
            baseline.deduplicated_findings
        )
        result.latest_severity_counts = self._count_by_severity(
            latest.deduplicated_findings
        )
        result.severity_delta = self._calculate_severity_delta(
            result.baseline_severity_counts, result.latest_severity_counts
        )
    ...
    def _calculate_trend_score(self, result: ComparisonResult) -> float:
        ...
        for diff in result.changed_findings:
            if diff.severity_improved:
                prev_weight = self.SEVERITY_TREND_WEIGHTS.get(
                    diff.previous_severity, 1.0
                )
                curr_weight = self.SEVERITY_TREND_WEIGHTS.get(
                    diff.finding.severity, 1.0
                )
    ...
    def generate_summary(self, result: ComparisonResult) -> str:
        ...
        # Scan info
        lines.append(
            f"Baseline Scan: {result.baseline_scan.scan_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        lines.append(
            f"Latest Scan:   {result.latest_scan.scan_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        lines.append(
            f"Overall Trend: {trend_emoji[result.trend_direction]} {result.trend_direction.value}"
        )
        ...
        lines.append(
            f"  {'Severity':<12} {'Baseline':>10} {'Latest':>10} {'Delta':>10}"
        )
        ...
        for severity in [
            ...
            lines.append(
                f"  {severity.value:<12} {baseline_count:>10} {latest_count:>10} {delta_str:>10}"
            )
        ...
        if result.critical_fixed > 0 or result.high_fixed > 0:
            lines.append(
                f"✅ Good news: {result.critical_fixed} CRITICAL and {result.high_fixed} HIGH findings fixed!"
            )
            lines.append("")
    ...
    def get_top_new_findings(
        self, result: ComparisonResult, limit: int = 10
    ) -> List[FindingDiff]:
        """
        ...
        """
        return sorted(
            result.new_findings, key=lambda f: f.finding.priority_score, reverse=True
        )[:limit]
    ...
    def get_top_fixed_findings(
        ...
        """
        return sorted(
            result.fixed_findings, key=lambda f: f.finding.priority_score, reverse=True
        )[:limit]
