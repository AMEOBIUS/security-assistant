...
class ScannerIntegrityValidator:
    ...
    REQUIRED_METHODS_BY_SCANNER = {
        ...
        "TrivyScanner": {
            "scan_filesystem",
            "scan_image",
        },  # Trivy has different method names
    }
    ...
    def validate_scanner(self, scanner: Any) -> SecurityValidationResult:
        ...
        if missing_methods:
            errors.append(f"Scanner missing required methods: {missing_methods}")
        ...
        required_methods = self.REQUIRED_METHODS_BY_SCANNER.get(
            scanner_name, {"scan_directory", "scan_file"}
        )
        ...
        for method_name in required_methods:
            if hasattr(scanner, method_name):
                ...
                if "scan_directory" in method_name or "scan_filesystem" in method_name:
                    ...
                    if (
                        "directory" not in params
                        and "directory_path" not in params
                        and "path" not in params
                    ):
                        warnings.append(
...
class ConfigurationSandbox:
    ...
    def validate_config_path(self, config_path: Path) -> SecurityValidationResult:
        ...
        for forbidden in self.FORBIDDEN_CONFIG_PATTERNS:
            if (
                forbidden in path_str_forward
                or forbidden in path_str_back
                or f"/{forbidden}/" in path_str_forward
                or f"\\{forbidden}\\" in path_str_back
            ):
                errors.append(
        ...
        if not errors:
            try:
                ...
                warnings.append(f"Config file is outside project root: {abs_path}")
        ...
        if not errors and not is_outside_root:
            ...
            rel_dir = (
                config_dir.relative_to(self.project_root)
                if config_dir != self.project_root
                else Path(".")
            )
            is_allowed = False
        ...
        if str(abs_path) in self._validated_configs:
            ...
            if current_hash != stored_hash:
                warnings.append("Config file has been modified since last validation")
